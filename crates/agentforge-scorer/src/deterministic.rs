use agentforge_core::{
    AgentFile, DimensionScore, Result, Scenario, ScoringMethod, ToolCallStep, Trace, TraceStep,
};
...
pub fn run_deterministic_checks(
    ...
    if result.tool_selection.value < 1.0 {
        result.failure_reasons.push(format!(
            "Tool selection failed (score={:.2})",
            result.tool_selection.value
        ));
    }
    ...
    if result.argument_correctness.value < 1.0 {
        result.failure_reasons.push(format!(
            "Argument correctness failed (score={:.2})",
            result.argument_correctness.value
        ));
    }
    ...
    if result.schema_compliance.value < 1.0 {
        result.failure_reasons.push(format!(
            "Schema compliance failed (score={:.2})",
            result.schema_compliance.value
        ));
    }
    ...
    if result.instruction_adherence.value < 1.0 {
        result.failure_reasons.push(format!(
            "Instruction adherence failed (score={:.2})",
            result.instruction_adherence.value
        ));
    }
...
fn check_tool_selection(trace: &Trace, scenario: &Scenario) -> DimensionScore {
    let required_tools: Vec<&str> = scenario
        .expected
        .tool_calls
        .iter()
    ...
    let called_tools: Vec<&str> = trace
        .steps
        .iter()
        .filter_map(|s| {
...
fn check_argument_correctness(trace: &Trace, scenario: &Scenario) -> DimensionScore {
    let tool_calls: Vec<&ToolCallStep> = trace
        .steps
        .iter()
        .filter_map(|s| {
            if let TraceStep::ToolCall(tc) = s {
        ...
        if let Some(exp) = expected {
            if let Some(arg_schema) = &exp.argument_schema {
                ...
                match validate_against_schema(&tc_step.arguments, arg_schema) {
                    ...
                    Err(e) => {
                        failure_details
                            .push(format!("Tool '{}' schema error: {}", tc_step.tool_name, e));
                    }
                }
            } else {
    ...
    if expected_min == 0 {
        ...
        return DimensionScore {
            ...
            rationale: Some(format!(
                "{} LLM calls for a no-tool scenario",
                llm_call_count
            )),
        };
    }
...
mod tests {
    use super::*;
    use agentforge_core::{
        DifficultyTier, ExpectedToolCall, FailureCluster, FinalOutputStep, ModelConfig,
        ModelProvider, ScenarioExpected, ScenarioInput, ScenarioSource, ToolCallStep, TraceStatus,
    };
    use chrono::Utc;
    use uuid::Uuid;
    ...
    fn schema_compliance_fails_for_missing_required_field() {
        let scenario = make_scenario("get_order", false);
        let mut trace =
            make_trace_with_tool_call("get_order", serde_json::json!({"id": "ORD-123"}));
