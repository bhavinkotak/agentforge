...
pub async fn promote_run(
    ...
    let challenger_version = agent_repo
        .find_by_id(run.agent_id)
        .await
        .map_err(|e| match e {
            AgentForgeError::NotFound { .. } => {
                ApiError::not_found(format!("Agent {} not found", run.agent_id))
            }
            other => ApiError::internal(other.to_string()),
        })?;

    // Find current champion by agent name
    let champion_versions = agent_repo
        .list_by_name(&challenger_version.name)
        .await
        .map_err(|e| ApiError::internal(e.to_string()))?;
    let champion = champion_versions.iter().find(|v| v.is_champion);

    let challenger_traces = trace_repo
        .list_by_run(run_id)
        .await
        .map_err(|e| ApiError::internal(e.to_string()))?;
    ...
    let champion_passing_scenarios = if let Some(champ) = champion {
        ...
        if let Some(champ_run) = runs.into_iter().next() {
            trace_repo
                .list_passing_scenario_ids(champ_run.id)
                .await
                .map_err(|e| ApiError::internal(e.to_string()))?
        } else {
            vec![]
    ...
    if decision.approved {
        agent_repo
            .set_champion(challenger_version.id, &challenger_version.name)
            .await
            .map_err(|e| ApiError::internal(e.to_string()))?;
        agent_repo
            .update_changelog(challenger_version.id, &decision.changelog)
            .await
            .map_err(|e| ApiError::internal(e.to_string()))?;
