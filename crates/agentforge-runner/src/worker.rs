...
pub fn summarize_batch(traces: &[Trace]) -> BatchSummary {
    ...
    let passed = traces.iter()
        .filter(|t| t.status == agentforge_core::TraceStatus::Pass)
        .count() as u32;
    let failed = traces.iter()
        .filter(|t| t.status == agentforge_core::TraceStatus::Fail)
        .count() as u32;
    let errors = traces.iter()
        .filter(|t| t.status == agentforge_core::TraceStatus::Error)
        .count() as u32;
    let review = traces.iter()
        .filter(|t| t.review_needed)
        .count() as u32;
    let total_tokens: u64 = traces.iter()
        .map(|t| (t.input_tokens + t.output_tokens) as u64)
        .sum();

    BatchSummary { total, passed, failed, errors, review, total_tokens }
}
...
mod tests {
    ...
    fn pass_rate_zero_when_empty() {
        let summary = BatchSummary {
            total: 0, passed: 0, failed: 0, errors: 0, review: 0, total_tokens: 0,
        };
    ...
    fn pass_rate_correct() {
        let summary = BatchSummary {
            total: 10, passed: 7, failed: 2, errors: 1, review: 0, total_tokens: 1000,
        };
